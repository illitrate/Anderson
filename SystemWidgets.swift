    ...
    private func updateCPU() {
        ...
        if result == KERN_SUCCESS {
            ...
            for i in 0..<Int(numCPUs) {
                ...
                totalUser += infoArray[Int(CPU_STATE_USER)]
                totalSystem += infoArray[Int(CPU_STATE_SYSTEM)]
                totalIdle += infoArray[Int(CPU_STATE_IDLE)]
            }
            
            let total = totalUser + totalSystem + totalIdle
            if total > 0 {
                cpuUsage = Double(totalUser + totalSystem) / Double(total) * 100
            }
        }
    }
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            VStack(spacing: 6) {
                ...
                Text(String(format: "%.0f%%", (usedMemory / totalMemory) * 100))
                    .font(MatrixTheme.font(size: 18, bold: true))
                    .foregroundColor(MatrixTheme.dimGreen)
                
                ProgressBar(value: usedMemory / totalMemory)
                    .frame(height: 8)
            }
        }
        .onAppear {
...
struct StorageWidget: View {
    ...
    private var displayedVolumes: [VolumeInfo] {
        ...
        volumes.filter { volume in
            volume.recentChange > preferences.diskSpaceChangeThreshold * 1_073_741_824 // GB to bytes
        }
    }
    ...
    private func updateVolumes() {
        ...
        guard let keys = try? fileManager.mountedVolumeURLs(includingResourceValuesForKeys: [
            .volumeNameKey,
